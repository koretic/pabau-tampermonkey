...
// @version      2026-07-10
// @description  Comprova si el client té el CI firmat segons el tractament seleccionat
...
// @require      file:///home/vant/Escritorio/Documents_projectes/pabau/tampermonkey/treatments_config.js
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
    ...
    const documentsApi = (() => {
        /**
         * Cerca un document pel nom exacte dins els documents del client.
         * @returns {Promise<{found: boolean, document: object|null}>}
         */
                    ...
                    onload: (res) => {
                        if (res.status === 200) {
                            try {
                                ...
                                resolve({ found: !!match, document: match || null });
                            } catch (e) {
    ...
    const buttonGuard = (() => {
        ...
        function block(customLabel) {
            ...
            // Si ja està bloquejat amb el mateix text, no fem res.
            if (
    ...
    const invoiceGuard = (() => {
        ...
        /** Comprova si el document està caducat. */
        function isExpired(createdAt, expiryMonths) {
            if (!createdAt) return true;
            const created = new Date(createdAt);
            if (Number.isNaN(created.getTime())) return true;
            const expiry = new Date(created);
            expiry.setMonth(expiry.getMonth() + Number(expiryMonths || 12));
            return new Date() > expiry;
        }

        /**
         * Resol quin document toca per al tractament seleccionat.
         * Retorna `null` si no hi ha mapeig.
         */
        function resolveTreatment() {
            const T = typeof unsafeWindow !== "undefined"
                ? unsafeWindow.PabauTreatments
                : window.PabauTreatments;
            if (!T || typeof T.resolve !== "function") return null;
            const key = { id: invoiceStore.id, name: invoiceStore.name };
            if (!key.id && !key.name) return null;
            return T.resolve(key);
        }

        /**
         * Crida única a l'API per (client + tractament); emmagatzema el resultat.
         */
        async function fetchAndDecide({ apiKey: key, clientId, viewK, resolved }) {
            try {
                const { found, document } = await documentsApi.findDocument({
                    apiKey: key,
                    clientId,
                ...
                const createdAt =
                    ...
                    document?.created;
                const expired = found
                    ? isExpired(createdAt, resolved.expiryMonths)
                    : true;
                console.log(
                    `[Pabau LOPD] Client ${clientId} · ${resolved.documentName} →`,
                    { found, expired, createdAt },
                );
                processedViews.add(viewK);
                return { shouldBlock: !found || expired, expired };
            } catch (err) {
                // Error de xarxa: no bloquejem per evitar falsos positius.
                console.error("[Pabau LOPD] Error consultant API:", err);
                processedViews.add(viewK);
                return { shouldBlock: false, expired: false };
            }
        }
        ...
        async function process({ apiKey: key, clientId }) {
            ...
            // 1. Resolem quin document toca segons el tractament seleccionat.
            const resolved = resolveTreatment();
            if (!resolved) {
                // Sense mapeig → no bloquejem (l'usuari decideix la política).
                return;
            }

            // 2. Clau única per (client + tractament). Si l'usuari canvia
            //    el tractament dins la mateixa vista, tornem a comprovar.
            const treatmentKey = invoiceStore.id || invoiceStore.name || "unknown";
            const keyV = `${clientId}|${treatmentKey}`;

            // Mateixa instància de botó i mateix tractament → res a fer.
            if (btn.dataset.lopdKey === keyV) return;

            // 3. Si ja hem fet la consulta per aquest tractament, reapliquem
            //    només l'estat del botó (ràpid, sense crida a l'API).
            ...
            // 4. Consulta a l'API + decisió.
            const { shouldBlock, expired } = await fetchAndDecide({
                apiKey: key,
                clientId,
                viewK: keyV,
                resolved,
